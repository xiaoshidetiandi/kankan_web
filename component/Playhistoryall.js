...
export default class Playhistoryall extends Component {
  constructor(props) {
    super(props);
    ...
    Alert.alert('是否删除?', '', [
      {text: '取消', onPress: () => {}},
      {
        ...
        onPress: () => {
          ...
          var playHistory = token.playHistory;
          for (let i = 0; i < playHistory.length; i++) {
            if (playHistory[i].mid == data.mid) {
              playHistory.splice(i, 1);
              break;
            }
          }
          token.playHistory = playHistory;
          var arr1 = [];
          var str = '';
  ...
  render() {
    return (
      <FlatList
        ...
        data={this.state.token.playHistory}
